...
<body>
                ...
                <ul class="nav nav-pills nav-sidebar flex-column nav-child-indent text-sm" data-widget="treeview"
                    ...
                    <li class="nav-item has-treeview  ">
                        ...
                        <ul class="nav nav-treeview" style="display: block">
                            ...
                            <li class="nav-item"><a class="nav-link active" href="/post/real-estate"
    ...
    <div class="content-wrapper">
        ...
        <section class="content">
            <div class="card-footer text-sm sticky-top">
                <a class="btn btn-sm bg-gradient-primary text-white" href="/post/real-estate/create"
            ...
            <div class="card card-primary card-outline text-sm mb-0">
                <div class="card-header">
                    <h3 class="card-title">Danh sách Thủ tục nhà đất</h3>
                </div>
                        ...
                        <tbody>
                            @foreach ($real_estate as $item)
                                <tr>
                                    <td class="align-middle">
                                        <div class="custom-control custom-checkbox my-checkbox">
                                            <input type="checkbox" class="custom-control-input select-checkbox"
                                                id="select-checkbox-377" value="377">
                                            <label for="select-checkbox-377" class="custom-control-label"></label>
                                        </div>
                                    </td>
                                    <td class="align-middle">
                                    ...
                                    <td class="align-middle">
                                        <a href="/post/real-estate/edit/{{ $item->id }}"
                                            title="{{ $item->tittle }}"><img class="rounded img-preview"
                                                src="{{ URL::asset('products/' . $item->image) }}"
                                                alt="{{ $item->tittle }}"></a>
                                    </td>
                                    <td class="align-middle">
                                        <a class="text-dark" href="/post/real-estate/edit/{{ $item->id }}"
                                            title="{{ $item->tittle }}">{{ $item->tittle }}</a>
                                        <div class="tool-action mt-2 w-clear">
                                            <a class="text-primary mr-3" href="/{{ $item->link }}" target="_blank"
                                                title="5 laptop bán tốt giá dưới 20 triệu đồng"><i
                                                    class="far fa-eye mr-1"></i>View</a>
                                            <a class="text-info mr-3"
                                                href="/post/real-estate/edit/{{ $item->id }}"
                                                title="{{ $item->tittle }}"><i class="far fa-edit mr-1"></i>Edit</a>
                                            <div class="dropdown">
                                                ...
                                                <ul aria-labelledby="dropdownCopy"
                                                    ...
                                                    <li><a href="#" class="dropdown-item copy-now"
                                                            data-id="377" data-table="news" data-copyimg="1"><i
                                                                class="far fa-caret-square-right text-secondary mr-2"></i>Sao
                                                            chép ngay</a></li>
                                                    <li><a href="/post/real-estate/edit/{{ $item->id }}"
                                    ...
                                    <td class="align-middle text-center">
                                        <div class="custom-control custom-checkbox my-checkbox">
                                            <input type="checkbox" class="custom-control-input show-checkbox"
                                                id="show-checkbox-noibat-377" data-table="news"
                                                data-id="{{ $item->id }}" data-loai="outstand"
                                                {{ $item->outstand == 1 ? 'checked' : '' }}
                                                onchange="updateDisplayOutstand(this)">
                                            <label for="show-checkbox-noibat-377"
                                    ...
                                    <td class="align-middle text-center">
                                        <div class="custom-control custom-checkbox my-checkbox">
                                            <input type="checkbox" class="custom-control-input show-checkbox"
                                                id="show-checkbox-377" data-table="news"
                                                data-id="{{ $item->id }}" data-loai="display"
                                                {{ $item->display == 1 ? 'checked' : '' }}
                                                onchange="updateDisplayStatus(this)">
                                            <label for="show-checkbox-377" class="custom-control-label"></label>
                                        </div>
                                    </td>
            ...
            <div class="card-footer text-sm">
                <a class="btn btn-sm bg-gradient-primary text-white" href="/post/news/create" title="Thêm mới"><i
    ...
    <script>
        function updateDisplayStatus(checkbox) {
            ...
            $.ajax({
                url: '/update-display-status-real-estate/' + categoryId,
                method: 'POST',
                data: {
                    _token: '{{ csrf_token() }}',
    ...
    <script>
        function updateDisplayOutstand(checkbox) {
            ...
            // Gửi yêu cầu Ajax để cập nhật trạng thái vào cơ sở dữ liệu
            $.ajax({
                url: '/update-display-outstand-real-estate/' + categoryId,
                method: 'POST',
                data: {
                    _token: '{{ csrf_token() }}',
    ...
    <script>
        $('.update-stt').change(function() {
            ...
            $.ajax({
                type: 'POST',
                url: '/post/real-estate', // Thay đổi thành URL phù hợp với route của bạn
                data: {
                    '_token': '{{ csrf_token() }}',
                    'item_id': item_id,
